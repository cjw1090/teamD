...
def process_image(frame):
    ...
    _, lane = cv2.threshold(L, 180, 255, cv2.THRESH_BINARY)
    ###################################################################
    
    # canny edge
    low_threshold = 60
    high_threshold = 70
    edge_img = cv2.Canny(lane, low_threshold, high_threshold)
    
    # HoughLinesP
    roi = edge_img[Offset : Offset+Gap, 0 : Width]
    all_lines = cv2.HoughLinesP(roi,1,math.pi/180,30,50,10)
    cv2.imshow('edge', roi)
...
def start():
    ...
    while True:
        ...
        #중간 속도
          ...
          drive(angle, 20)
          flag = False
        elif(flag == False and(lpos == 0 or rpos == Width )):
          pid = PID(0.2, 0, 0.02)
          angle = pid.pid_control(error)
          drive(angle, 15)
        else:
