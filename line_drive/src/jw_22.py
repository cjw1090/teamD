...
class ParamFindHelper:
    ...
    def __init__(self, video_name):
        ...
        cv2.createTrackbar("MinLineLength", "main", 20, 300, lambda x: x)
        cv2.createTrackbar("MaxLineGap", "main", 10, 300, lambda x: x)
    ...
    def processHough(self, minLineLength=5, maxLineGap=20):
        #minLineLength = cv2.getTrackbarPos("MinLineLength", "main")
        #maxLineGap = cv2.getTrackbarPos("MaxLineGap", "main")
    ...
    def determineDirection(self):
        ...
        # if fails, use previous cross point
            ...
            if lmr.get('left', None):
                x, y = self.cross_point_history[-1]
                x = self.width
                cross_point = (x, y)
            elif lmr.get('right', None):
                x, y = self.cross_point_history[-1]
                x = 0
                cross_point = (x, y)
            else:
    ...
    def findLmr(self, lines):
        ...
        for line in lines:
            ...
            # find left line
            if a < 0 and max(x1, x2) < self.width * 4.9 / 10:
            ...
            # find right line
            elif a > 0 and min(x1, x2) > self.width*5.1/10:
            ...
            # find middle line
            """
            elif (min(x1, x2) > self.width * 45 / 100) and (max(x1, x2) < self.width * 55 / 100):
