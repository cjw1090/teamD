...
def callback(msg):
    global arData
    global ar_parking_flag, ar_drive_flag, temp
...
def parking_dirve():
    ...
    global arData
    if abs(pitch) < 2 and abs(arData["DX"]) < 0.03:
        angle = 0
    ...
    # left
        ...
        if (pitch < -2 and arData["DX"] <-0.03 and arData["DZ"] < 0.4) or (arData["DZ"] < 0.4 and pitch > -2):
            back_drive(50, 30)
    ...
    if abs(pitch) < 2 and arData["DZ"] <= 0.30:
        print("stop")
        speed=0
